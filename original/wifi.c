...
bool dataAnalyzeFlag = TRUE;  // 设置是否对接收到的数据进行解析，默认解析

//uint8_t data[512];

/* 用户数据处理函数
* 传入参数：用户数据数组，数据长度，控制数据，地址数据
* 提示：当dataAnalyzeFlag = FALSE时控制数据和地址数据默认为0
*/
...
void clientStart(void)
{
	...
	{
		if(Send_AT_commend("AT+CWMODE=1", &modeFlag, 500))
			;
...
void serverStart(void)
{
	...
	{
		if(Send_AT_commend("AT+CWMODE=2", &modeFlag, 500))
				;
		if(Send_AT_commend("AT+RST", &rstFlag, 3000))
				rstFlag = FALSE;
	}	
...
void recDataHandle(void)
{
	...
	if(Espdatatype.AtRecFlag == 1)
	{
		...
		{
			char *addr = strstr((char *)Espdatatype.AtBuffer, "192");
			uint8_t counter = 0;
			if(*addr)
			{
				printf("TCP服务器IP地址：");
...
void recDataAnalyze(uint8_t *recData)
{
	...
	uint8_t cs = 0;
	if(dataAnalyzeFlag) // 判断是否对接收到的数据进行解析
	...
	else
	{
		...
		uint8_t *userData = (uint8_t *)malloc(sizeof(uint8_t)*(counter+1));
		for(int i=0;i<counter;i++)
			userData[i] = recData[i];
		userData[counter] = '\0';  //	结尾添加结束符
		userToDo(userData, length-4, con, addr);
		free(userData);
	}
}
...
uint8_t wifiStart(void)
{
	...
	printf("*****************************************\r\n");
	printf("*                                       *\r\n");
	printf("*  JHX_ESP8266_PACKAGE    version 1.5   *\r\n");
	printf("*                                       *\r\n");
	printf("*****************************************\r\n");
	for(int i=0; i<=3; i++)
	{
